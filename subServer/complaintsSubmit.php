...
if (isset($_POST['submit'])) {
    ...
    //validation for Area of Concern
    if (empty($_POST['concern'])) {
        ?>
            <script>
                alert("Please select area of concern!");
                window.location.href="../contact.php";
            </script>
        <?php
            exit;
    ...
    try {
        $query = "INSERT into complaints_tlb (name,email,contact,hospital,concern,remark,date) 
                values('$name','$email','$contact','$hospital','$concern','$remark',curdate())";
